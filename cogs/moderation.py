...
class moderation(commands.Cog):
    ...
    async def broadcast(self, ctx, *, message: str):
        ...
        embed = base_embed(self.bot.cache.instance(ctx.author.id, ctx.guild.id).id, title="Message broadcasted", description=res)
        await ctx.send(embed=embed)
    ...
    async def skip_match(self, ctx, *, map_name: str = ""):
        ...
        embed = base_embed(self.bot.cache.instance(ctx.author.id, ctx.guild.id).id, title="Skipped the current match", description=res)
        await ctx.send(embed=embed)
    ...
    async def restart_match(self, ctx):
        ...
        embed = base_embed(self.bot.cache.instance(ctx.author.id, ctx.guild.id).id, title="Restarted the current match", description=res)
        await ctx.send(embed=embed)
